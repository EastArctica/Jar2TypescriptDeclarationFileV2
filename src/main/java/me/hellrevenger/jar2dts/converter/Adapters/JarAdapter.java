...
public class JarAdapter extends BaseAdapter {
    private final Jar jar;

    public JarAdapter(File file) throws IOException {
        super(file);
        jar = new JarAdapter.Jar(file);
    }
    ...
    public byte[] getFileData(String filename) {
        try {
            return jar.getFileData(filename);
        } catch (IOException e) {
    ...
    public InputStream getInputStream(String filename) {
        try {
            return jar.getInputStream(jar.getJarEntry(filename));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }

    private static class Jar extends JarFile {
